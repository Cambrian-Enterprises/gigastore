	...
	public static function get_sample_packages( $package = null ) {
		...
		if ( ! $sample_packages ) {
			...
			if ( 'sample-data.json' == self::$server ) {
				...
				$sample_packages = ( $buffer = file_get_contents($sample_packages) ) ? $buffer : $wp_filesystem->get_contents($sample_packages);
			}
						...
						if ( $results ) {
							foreach ( $results as $result ) {
								...
								if ( strlen( $queries . $query ) > self::$max_backup_file_size ) {
									...
									file_put_contents($path, $queries) || $wp_filesystem->put_contents($path, $queries);
		...
		if ( ! is_dir( $path ) ) {
			...
			while ( count( $results ) ) {
				...
				if ( ! is_dir( $currentDir = implode('/', $path) ) ) {
					mkdir($currentDir, 0755) || $wp_filesystem->mkdir($currentDir, 0755);
				}
